...
public:
  MinimalAnalysis(){
    cout << "Creating analysis class instance." << endl; 

    // Setup the file reader instance.
    h22chain = new TChain("h22"); 
  }

  ~MinimalAnalysis(){
    cout << "Destroying analysis class instance." << endl; 
  ...
  void AddFiles(std::string inputListOfFiles, int numberOfFiles){
    ...
    for(int i=0; i<numberOfFiles; i++){
      ...
      inputFile >> currentFilename;
      h22chain->Add(currentFilename.c_str());
    }
  }
  
  void Run(){
    cout << "Running minimal analysis... " << endl; 

    if (h22chain->GetEntries() > 0){    
...
void minimalClass(){
  ...
  analysis->AddFiles("../requiredFiles/lists/data.dat", 10); 
  analysis->Run(); 
} 
