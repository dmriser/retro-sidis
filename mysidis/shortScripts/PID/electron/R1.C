...
*/
 ...
 for(int s = 0; s < 6; s++){
   ...
   hD[1][s] = (TH2F*) fD->Get(Form("eIDplots/tl1_xVtl1_y_e_hist_c%i_s%i", 1, s+1));
   hD[1][s]->GetXaxis()->SetTitle("Y (cm)");
   hD[1][s]->GetYaxis()->SetTitle("X (cm)");
   hD[1][s]->SetTitle(Form("Sect. %i (data)", s+1));
   hD[1][s]->GetXaxis()->SetRangeUser(-25, 25);
   hD[1][s]->GetYaxis()->SetRangeUser(21, 57);
   hD[1][s]->Draw("colz");
 ...
 for(int s = 0; s < 6; s++){
   ...
   hMC[1][s] = (TH2F*) fMC->Get(Form("eIDplots/tl1_xVtl1_y_e_hist_c%i_s%i", 1, s+1));
   hMC[1][s]->GetXaxis()->SetTitle("Y (cm)");
   hMC[1][s]->GetYaxis()->SetTitle("X (cm)");
   hMC[1][s]->SetTitle(Form("Sect. %i (MC)", s+1));
   hMC[1][s]->GetXaxis()->SetRangeUser(-25, 25);
   hMC[1][s]->GetYaxis()->SetRangeUser(21, 57);
   hMC[1][s]->Draw("colz");
 ...
 can->Print("/volatile/clas12/dmriser/plots/nathan/eid/region1.pdf");
