{
  ...
  const std::string input_file_dir("/u/home/dmriser/mysidis-histos");
  const std::string output_dir("/volatile/clas12/dmriser/plots/nathan/eid");

  TFile *fD = new TFile(Form("%s/pid.data.s1.n12114.root", input_file_dir.c_str()));
  TFile *fMC = new TFile(Form("%s/pid.MonteCarlo_v12.s1.n32255.root", input_file_dir.c_str()));
